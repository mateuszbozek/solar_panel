class Joint
  ...
  def self.calculate_coordinates(is_next, gap_on_horizontal, gap_on_vertical, is_last_row, panel)
      # p "                   Wewnątrz calculate_joints - panel : #{panel.x}   -   #{is_next}-      #{gap_on_horizontal}-   #{gap_on_vertical}-    #{is_last_row}"
      if is_next && !( panel.is_under || panel.is_diagonally )
      ...
      elsif is_next && ( panel.is_under || panel.is_diagonally )
      ...
      elsif !is_next && panel.is_diagonally
